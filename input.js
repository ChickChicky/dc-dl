        ...
        r => {
            ...
            stdin.once( 'data', (d) => {
                if (!raw) {
                    ...
                    else if (d == '\x08' || d == '\x7f') r('backspace');
...
async function input(prompt,x,y,replace,defaultValue,placeHolder='',exitChars=cancelChars,update=()=>undefined) {
    ...
    async function DSR() {
        ...
        let v;
        while (!v) {
            let vv = ((await getch('utf-8')).match(/\x1b\[(\d*);(\d*)R/)??[]).slice(1).map(n=>+(n|'0'));
            v = vv.length ? vv : null;
        }
        return Array.from(v);
    }
    ...
    while (true) {
        ...
        } else if (ch == Key.backspace || ch == Key.backspace2) {
        ...
        await update(val,ch);
        process.stdout.write(resetPos + prePrompt + (typeof prompt == 'function' ? await prompt(val) : prompt) + value + ((ch == '\b' || ch == '\x1b[3~' || ch == '\x7f')?' ':'') + ((ch != '\r') ? `\x1b[${(stripVTCC(value).length-cur+(ch == '\b' || ch == '\x1b[3~' || ch == '\x7f'))?`\x1b[${stripVTCC(value).length-cur+(ch == '\b' || ch == '\x1b[3~' || ch == '\x7f')}D`:''}` : ''));
        if (ch == '\r') {
